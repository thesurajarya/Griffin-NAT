...
const Home = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setError(
        "Failed to connect to the analysis engine. Is the backend running?",
      );
      setIsUploading(false);
    }
  };
        ...
        <div className="bg-slate-900/80 backdrop-blur-md rounded-xl shadow-2xl shadow-black/50 p-8 w-full max-w-2xl border border-slate-700/50">
          <label
            ...
            className={`flex flex-col items-center justify-center w-full h-64 border-2 border-dashed rounded-lg cursor-pointer transition-all duration-300
              ${file ? "border-blue-500 bg-slate-800/80" : "border-slate-500 hover:border-blue-400 hover:bg-slate-800/50"}`}
          >
          ...
          <div className="mt-8 flex justify-center">
            {/* ClickSpark now properly wraps the button */}
            <ClickSpark
              sparkColor="#fff"
              sparkSize={12}
              sparkRadius={20}
